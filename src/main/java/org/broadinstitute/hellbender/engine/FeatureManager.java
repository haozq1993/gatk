...
public final class FeatureManager implements AutoCloseable {
     ...
     * @return the codec suitable for decoding the provided file
     */
    public static FeatureCodec<? extends Feature, ?> getCodecForFile( final Path featureFile, final Class<? extends Feature> featureType ) {
        // Make sure file exists/is readable
        if ( ! Files.isReadable(featureFile) ) {
        ...
        if ( candidateCodecs.size() > 1 ) {
            ...
            throw new GATKException("Multiple codecs found able to decode file " + featureFile.toAbsolutePath().toUri() +
        ...
        logger.info("Using codec " + selectedCodec.getClass().getSimpleName() + " to read file " + featureFile.toAbsolutePath().toUri());
